...
*/
// file asli jquery.wdi-menueditor.js
	...
	$.extend($.fn.np.prototype, {
		customInit : function() 
		{
			...
			list_container.on('click', 'button', function(e) {
				...
				if (action === 'viewdetail') {
					plugin.options.viewDetailBtnCall($list);
				}
				// batas tambahan eko
			});

			// tambahan eko untuk collapseAll pada waktu load
			this.collapseAll();
			// batas tambahan eko untuk collapseAll pada waktu load
		}, 
		...
		addOptions: function() {
			var addOptions = {
				...
				// tambahan eko
				viewDetailBtnHTML 	: '<button data-action="viewdetail" type="button" class="fas fa-eye pull-right"></button>',
				viewDetailBtnCall	: function(){},
				// batas tambahan eko
		...
		addButton: function () {
			...
			$.each(this.el.find(this.options.itemNodeName), function(i, el) {
				...
				//tambahan eko
				$viewDetailBtn = $(options.viewDetailBtnHTML);
				$(el).prepend($viewDetailBtn);
				// batas tambahan eko
				$(el).prepend($editBtn);
