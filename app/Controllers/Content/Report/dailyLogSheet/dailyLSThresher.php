...
use App\Models\Content\Report\dailyLogSheet\dailyLSThresherModel;
use \Config\App;
...
class dailyLSThresher extends \App\Controllers\BaseController
{
    public $dailyLSThresherModel;

	public function __construct() {

		parent::__construct();
		
		$this->dailyLSThresherModel = new dailyLSThresherModel;
	...
	public function index(){
		...
		$this->view('Content/Report/dailyLogSheet/dailyLSThresher/dailyLSThresherView.php', $data);
	}

    public function getStationID(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->dailyLSThresherModel->getStationID());     
    }

    public function dataList(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->dailyLSThresherModel->dataList());        
	}
    ...
    public function cekDataexportExcelFile(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->dailyLSThresherModel->dataListExcel());        
	}
    ...
    public function exportPDFFile()
    {
        ...
        $titleOrg =$this->dailyLSThresherModel->getSCD_MA_PARAM('ORG','ORGPRN')[0]['VALSTR'];
        $titleSite =$this->dailyLSThresherModel->getSCD_MA_PARAM('ORG','ORGSITELONG')[0]['VALSTR'];

        $data['imagesPath'] = $this->config->imagesPath;
		$data['Judul'] = 'Laporan '.$this->data['site_title'];
		$data['data_sql'] = $this->dailyLSThresherModel->dataListExcel();
        ...
        <tr>
            ...
            <td style="width:33.33%; text-align:center;">  
                THRESHER & EMPTY BUNCH PRESS STATION <br> DAILY LOGSHEET
            </td>
        ...
        $html = view('Content/Report/dailyLogSheet/dailyLSThresher/pdfView.php', $data);
        $mpdf->WriteHTML($html);
	...
	public function cekPdfView(){
		...
		echo view('Content/Report/dailyLogSheet/dailyLSThresher/pdfView.php', $data);
