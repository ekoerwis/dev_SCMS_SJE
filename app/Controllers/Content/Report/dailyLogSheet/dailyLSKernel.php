...
use App\Models\Content\Report\dailyLogSheet\dailyLSKernelModel;
use \Config\App;
...
class dailyLSKernel extends \App\Controllers\BaseController
{
    public $dailyLSKernelModel;

	public function __construct() {

		parent::__construct();
		
		$this->dailyLSKernelModel = new dailyLSKernelModel;

		$this->data['site_title'] = 'LogSheet Kernel ';
	...
	public function index(){
		...
		$this->view('Content/Report/dailyLogSheet/dailyLSKernel/dailyLSKernelView.php', $data);
	}

    public function getStationID(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->dailyLSKernelModel->getStationID());     
    }

    public function dataList(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->dailyLSKernelModel->dataList());        
	}
    ...
    public function cekDataexportExcelFile(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->dailyLSKernelModel->dataListExcel());        
	}
    ...
    public function exportPDFFile()
    {
        ...
        $titleOrg =$this->dailyLSKernelModel->getSCD_MA_PARAM('ORG','ORGPRN')[0]['VALSTR'];
        $titleSite =$this->dailyLSKernelModel->getSCD_MA_PARAM('ORG','ORGSITELONG')[0]['VALSTR'];

        $data['imagesPath'] = $this->config->imagesPath;
		$data['Judul'] = 'Laporan '.$this->data['site_title'];
		$data['data_sql'] = $this->dailyLSKernelModel->dataListExcel();
        ...
        <tr>
            ...
            <td style="width:33.33%; text-align:center;">  
                KERNEL STATION <br> DAILY LOGSHEET
            </td>
        ...
        $html = view('Content/Report/dailyLogSheet/dailyLSKernel/pdfView.php', $data);
        $mpdf->WriteHTML($html);
	...
	public function cekPdfView(){
		...
		echo view('Content/Report/dailyLogSheet/dailyLSKernel/pdfView.php', $data);
