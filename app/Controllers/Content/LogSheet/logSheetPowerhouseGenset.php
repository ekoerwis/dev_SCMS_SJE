...
use App\Models\Content\LogSheet\logSheetPowerhouseGensetModel;
use \Config\App;
...
class logSheetPowerhouseGenset extends \App\Controllers\BaseController
{
    public $logSheetPowerhouseGensetModel;

	public function __construct() {

		parent::__construct();
		
		$this->logSheetPowerhouseGensetModel = new logSheetPowerhouseGensetModel;
	...
	public function index(){
		...
		$this->view('Content/LogSheet/logSheetPowerhouseGenset/logSheetPowerhouseGensetView.php', $data);
	}

    public function getStationID(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetPowerhouseGensetModel->getStationID());     
    }

    public function dataList(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetPowerhouseGensetModel->dataList());        
	}
    ...
    public function cekDataexportExcelFile(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetPowerhouseGensetModel->dataListExcel());        
	}
    ...
    public function exportPDFFile()
    {
        ...
        $titleOrg =$this->logSheetPowerhouseGensetModel->getSCD_MA_PARAM('ORG','ORGPRN')[0]['VALSTR'];
        $titleSite =$this->logSheetPowerhouseGensetModel->getSCD_MA_PARAM('ORG','ORGSITELONG')[0]['VALSTR'];

		$data['Judul'] = 'Laporan '.$this->data['site_title'];
		$data['data_sql'] = $this->logSheetPowerhouseGensetModel->dataListExcel();
        ...
        $html = view('Content/LogSheet/logSheetPowerhouseGenset/pdfView.php', $data);
        $mpdf->WriteHTML($html);
	...
	public function cekPdfView(){
		...
		echo view('Content/LogSheet/logSheetPowerhouseGenset/pdfView.php', $data);
