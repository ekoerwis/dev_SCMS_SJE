...
use App\Models\Content\LogSheet\logSheetNetModel;
use \Config\App;
...
class logSheetNet extends \App\Controllers\BaseController
{
    public $logSheetNetModel;

	public function __construct() {

		parent::__construct();
		
		$this->logSheetNetModel = new logSheetNetModel;
	...
	public function index(){
		...
		$this->view('Content/LogSheet/logSheetNet/logSheetNetView.php', $data);
	}

    public function dataList(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetNetModel->dataList());        
	}
    ...
    public function cekDataexportExcelFile(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetNetModel->dataListExcel());        
	}

    public function getNetAdd(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetNetModel->getNetAdd());     
    }
	...
	public function exportExcelFile()
    {
        $this->cekHakAkses('READ_DATA');
        $data = $this->data;
        $data_parameter = $this->logSheetNetModel->dataListExcel();
        ...
        $titleOrg =$this->logSheetNetModel->getSCD_MA_PARAM('ORG','ORGPRN')[0]['VALSTR'];
        $titleSite =$this->logSheetNetModel->getSCD_MA_PARAM('ORG','ORGSITELONG')[0]['VALSTR'];
        

        $fileName = 'writable/filedownload/logSheetNet_'.$userid.'.xlsx';
    ...
    public function exportPDFFile()
    {
		...
		$data['data_sql'] = $this->logSheetNetModel->dataListExcel();
        ...
        $html = view('Content/LogSheet/logSheetNet/pdfView.php', $data);
        $mpdf->WriteHTML($html);
