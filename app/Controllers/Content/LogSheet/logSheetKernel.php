...
use App\Models\Content\LogSheet\logSheetKernelModel;
use \Config\App;
...
class logSheetKernel extends \App\Controllers\BaseController
{
    public $logSheetKernelModel;

	public function __construct() {

		parent::__construct();
		
		$this->logSheetKernelModel = new logSheetKernelModel;
	...
	public function index(){
		...
		$this->view('Content/LogSheet/logSheetKernel/logSheetKernelView.php', $data);
	}

    public function getStationID(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetKernelModel->getStationID());     
    }

    public function dataList(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetKernelModel->dataList());        
	}
    ...
    public function cekDataexportExcelFile(){
        $this->cekHakAkses('READ_DATA');
		echo json_encode($this->logSheetKernelModel->dataListExcel());        
	}
    ...
    public function exportPDFFile()
    {
        ...
        $titleOrg =$this->logSheetKernelModel->getSCD_MA_PARAM('ORG','ORGPRN')[0]['VALSTR'];
        $titleSite =$this->logSheetKernelModel->getSCD_MA_PARAM('ORG','ORGSITELONG')[0]['VALSTR'];

		$data['Judul'] = 'Laporan '.$this->data['site_title'];
		$data['data_sql'] = $this->logSheetKernelModel->dataListExcel();
        ...
        $html = view('Content/LogSheet/logSheetKernel/pdfView.php', $data);
        $mpdf->WriteHTML($html);
	...
	public function cekPdfView(){
		...
		echo view('Content/LogSheet/logSheetKernel/pdfView.php', $data);
