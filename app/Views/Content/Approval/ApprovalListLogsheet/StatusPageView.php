<style type="text/css">
    ...
    <!--- DATA GRID ------------------------------------------------------------------------->
    <table id="dg" title="" class="easyui-datagrid"  style="<?php  echo $tinggi_dg ; ?>"  url="<?php echo site_url() . '/../Content/Approval/ApprovalListLogsheet/dataListStatus?MONTHNUMBER='.$MONTHNUMBER.'&YEARNUMBER='.$YEARNUMBER.'&IDMODULE='.$IDMODULE; ?>" 
    ...
    <script type="text/javascript">
        ...
        function statusButton(value,row){
            var href = 'x';

            if(value == 1){
                result = '<button type="button" class="btn btn-success"> Finish </button>';
            } else {
                result = '<button type="button" class="btn btn-secondary"> On Progress </button>';
            }
